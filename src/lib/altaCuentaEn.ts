 ...
 *
 * Recibe el cliente porque también la usa el conector MCP (`api/`), que corre en
 * el servidor con el token de la persona y no puede importar el cliente del
 * navegador (`lib/supabase.ts` lee `import.meta.env` al cargarse). Por eso este
 * archivo no importa nada más que tipos.
 */
export async function crearCuentaConSaldoEn(
